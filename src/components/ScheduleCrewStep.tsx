...
export function ScheduleCrewStep({
  bookingData,
  updateBookingData,
}: ScheduleCrewStepProps) {
  const [loading, setLoading] = React.useState(false);
  const [error, setError] = React.useState("");
  const [success, setSuccess] = React.useState("");

  const updateCrewCount = (
    type: "photographers" | "cinematographers",
    increment: boolean
  ) => {
  ...
  const removeMainFunction = (index: number) => {
    const newFunctions = bookingData.mainFunctions.filter(
      (_, i) => i !== index
    );
  ...
  const updateMainFunction = (index: number, updates: any) => {
    ...
    if (updates.startTime || updates.endTime) {
      ...
      const { extraHours } = calculateMainFunctionHours(
        startTime,
        endTime,
        currentFunc.type
      );
  ...
  const addAdditionalFunction = (functionType: string) => {
    ...
    updateBookingData({
      additionalFunctions: [...bookingData.additionalFunctions, newFunction],
    });
  };

  const removeAdditionalFunction = (index: number) => {
    const newFunctions = bookingData.additionalFunctions.filter(
      (_, i) => i !== index
    );
  ...
  const calculateMainFunctionHours = (
    startTime: string,
    endTime: string,
    functionType?: string
  ) => {
    ...
    const isReducedCrewFunction =
      functionType &&
      ["birthday", "anniversary", "baptism", "noolukettu"].includes(
        functionType
      );
  ...
  React.useEffect(() => {
    ...
    const hasWeddingFunctions = bookingData.mainFunctions.some((func) =>
      ["engagement", "wedding", "weddingAndEngagement", "reception"].includes(
        func.type
      )
    );
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card className="bg-gray-100 border-gray-300 shadow-lg">
          <CardContent className="p-6">
            ...
            <Select
              value={bookingData.guestRange}
              onValueChange={(value) =>
                updateBookingData({ guestRange: value })
              }
            >
        ...
        <Card className="bg-white border-gray-300 shadow-lg">
          <CardContent className="p-6">
            ...
            <Select
              ...
              onValueChange={(value) =>
                updateBookingData({ budgetRange: value })
              }
            >
              <SelectTrigger className="border-gray-300 focus:border-black bg-white">
      ...
      <Card className="bg-gray-50 border-gray-300 shadow-lg">
        <CardHeader className="pb-4">
          ...
          <p className="text-sm text-gray-600">
            Select primary event functions (Minimum 8 hours + 1 hour
            complimentary, Default: 2 Photographers + 2 Cinematographers)
          </p>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
              {mainFunctionTypes
                .filter((func) => func.category === "wedding")
                .map((func) => (
                  <Button
                    key={func.id}
                    variant="outline"
                    ...
                    disabled={bookingData.mainFunctions.some(
                      (f) => f.type === func.id
                    )}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
              {mainFunctionTypes
                .filter((func) => func.category === "other")
                .map((func) => (
                  <Button
                    key={func.id}
                    variant="outline"
          ...
          {bookingData.mainFunctions.length > 0 && (
            <div className="space-y-4">
              ...
              {bookingData.mainFunctions.map((func, index) => {
                const funcInfo = mainFunctionTypes.find(
                  (f) => f.id === func.type
                );
                return (
                  <Card
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
                      <div>
                        ...
                        <Input
                          ...
                          value={func.date}
                          onChange={(e) =>
                            updateMainFunction(index, { date: e.target.value })
                          }
                        ...
                        <div className="col-span-2">
                          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                            ...
                            <p className="text-blue-700 text-xs mt-1">
                              No fixed time schedule required for newborn
                    ...
                    {func.type !== "newbornPhotography" &&
                      (() => {
                        const { totalHours, extraHours } =
                          calculateMainFunctionHours(
                            func.startTime,
                            func.endTime,
                            func.type
                          );
                        ...
                        const complimentaryTotal = isReducedCrewFunction
                          ? 7
                          : 9;
                          ...
                          <div className="bg-gray-100 border border-gray-300 rounded-lg p-3 mb-4">
                            ...
                            <div className="flex items-center justify-between text-sm mt-1">
                              <span className="text-gray-600">
                                Standard ({standardHours} hrs):
                              </span>
                              <span className="text-green-600">Included</span>
                            </div>
                            {totalHours > standardHours &&
                              totalHours <= complimentaryTotal && (
                                <div className="flex items-center justify-between text-sm mt-1">
                                  <span className="text-gray-600">
                                    Complimentary (+{totalHours - standardHours}{" "}
                                    hr):
                                  </span>
                            ...
                            {extraHours > 0 && (
                              <div className="flex items-center justify-between text-sm mt-1">
                                ...
                                <span className="text-orange-600 font-medium">
                                  {extraHours} hours (additional charges apply)
                                </span>
                              </div>
                            )}
                          </div>
                            ...
                            <Button
                              ...
                              onClick={() => {
                                const newCount = Math.max(
                                  1,
                                  func.photographers - 1
                                );
                                updateMainFunction(index, {
                                  photographers: newCount,
                                });
                              }}
                            ...
                            <div className="flex items-center space-x-2">
                              <Button
                                ...
                                onClick={() => {
                                  const newCount = Math.max(
                                    1,
                                    func.cinematographers - 1
                                  );
                                  updateMainFunction(index, {
                                    cinematographers: newCount,
                                  });
                                }}
                                className="h-8 w-8 p-0 border-gray-300 hover:bg-gray-100"
                              >
                          ...
                          <div>
                            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                              ...
                              <p className="text-yellow-700 text-xs mt-1">
                                No cinematographer needed for newborn sessions
                              </p>
                            </div>
                          </div>
                        )}
      ...
      <Card className="bg-white border-gray-300 shadow-lg">
        <CardHeader className="pb-4">
          ...
          <p className="text-sm text-gray-600">
            Add pre-wedding ceremonies and additional events (Minimum 4 hours +
            1 hour complimentary, Default: 1 Photographer + 1 Cinematographer)
          </p>
        ...
        <CardContent className="space-y-6">
          ...
          {bookingData.additionalFunctions.length > 0 && (
            <div className="space-y-4">
              <h4 className="text-black-elegant">
                Selected Additional Functions
              </h4>
              {bookingData.additionalFunctions.map((func, index) => {
                const funcInfo = additionalFunctionTypes.find(
                  (f) => f.id === func.type
                );
                return (
                  <Card
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
                      <div>
                        ...
                        <Input
                          ...
                          value={func.date}
                          onChange={(e) =>
                            updateAdditionalFunction(index, {
                              date: e.target.value,
                            })
                          }
                          className="border-gray-300 bg-white"
